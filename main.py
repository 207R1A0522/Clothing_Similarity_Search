...
def read_csv_file(file_path, text_column, url_column, rating_column):
    ...
    urls = []
    ratings = []
    with open(file_path, 'r') as csv_file:      #Opening file in Read only mode
...
def compute_similarity(input_text, database_texts):
    ...
    return similarity_scores[0]                                      # Returning similarity_scroes after calculation

# Get top N similar items based on cosine similarity scores
def get_top_similar_items(input_text, database_texts, database_ratings, N=50):
...
def compare_csv_with_input(csv_file_path, text_column, url_column, rating_column, input_text, N=50):
...
compare_csv_with_input(csv_file_path, text_column, url_column, rating_column, input_text, N=50)
